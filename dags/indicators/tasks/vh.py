...
def calc_vh(url):
    ...
    with rasterio.open(file) as src:
        ...
        with rasterio.open(os.path.join('/tmp', 'S1_VH_dB.tif'), 'w', **profile) as dst:
            dst.write(vh_db.astype(np.float32), 1)    
        
        result = upload_file_to_yandex(
            local_path='/tmp/S1_VH_dB.tif',
            yandex_object_name=f'gee_exports/{foldername}/vh_{metaname}', 
